...
**Source URL:** [https://help.gohighlevel.com/support/solutions/articles/155000004390-overview-of-merge-fields-custom-variables](https://help.gohighlevel.com/support/solutions/articles/155000004390-overview-of-merge-fields-custom-variables)  
...
When you build a contact-based workflow, you can select Company fields in the variable picker alongside contact fields. If the contact is associated with a Company record, workflow steps and conditions can pull company details (for example, company name, phone, email, website, and address). If the contact is not associated with a Company, company values resolve to blank.
...
For example, in the email builder you can use Conditional Send to show a block of content only when certain conditions are met. If you have one block with the first_name merge field (Hi {{contact.first_name}},) set to send when the first_name is not empty, and a second block without the merge field (a default value like "Hi Friend,") set to send when the first_name field is empty, then you get a similar effect.
